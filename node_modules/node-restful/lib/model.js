...
/**
 * Adds the default controllers for the HTTP methods and one to get the schema
 */
 ...
 *
 * @param {String|Object} path: absolute path (including method) or object of controllers
...
/**
 * Registers all of the controllers in routeObj to the given app
 *
...
// Will I still support handle()? I think maybe for default controllers it might be nice, but
...
Model.prototype.send = function(controllers, req, res, next) {
  var handler = this.controllers;
  req.quer = this.filter(req.filters, req.body, req.query, this.Model.find({}));
  req.templatePath = this.template(controllers, req.filters);
